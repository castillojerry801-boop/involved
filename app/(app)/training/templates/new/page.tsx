    ...
    <div className="mt-2 space-y-1">
      {sets.map((s, i) => (
        <div key={i} className="flex items-center gap-1.5">
          ...
          <select
            value={s.setType}
            onChange={e => update(i, 'setType', e.target.value)}
...
export default function NewTemplatePage() {
  ...
  useEffect(() => {
    fetch('/api/training/equipment')
      .then(r => r.json() as Promise<{ profiles: EquipmentProfile[] }>)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error ?? 'Failed to save template'); return
      }
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 md:px-8">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="font-black text-xl text-zinc-900 dark:text-white">New template</h1>
      </div>

      <div className="mb-6 space-y-3">
        <div>
          <label className="mb-1 block text-xs font-medium text-zinc-500">Template name *</label>
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Push Day, Hotel Workout"
        ...
        <div>
          ...
          <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Quick upper-body session"
            className="w-full rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-4 py-2.5 text-sm text-zinc-900 dark:text-white placeholder:text-zinc-400 focus:outline-none focus:border-zinc-400 dark:focus:border-zinc-500" />
        </div>
        {profiles.length > 0 && (
          <div>
